...
export function TilbakekrevingsvalgField() {
    ...
    return (
        <RadioGroup
            ...
            name={name}
            value={value}
            onChange={(valg: Tilbakekrevingsvalg) => {
                onChange(valg);
                // Fritekstfeltet demonteres når varsel ikke skal sendes. Uten opprydding blir feilmeldingen
                // stående igjen i feilsammendraget frem til neste innsending.
                if (valg !== Tilbakekrevingsvalg.OPPRETT_TILBAKEKREVING_MED_VARSEL) {
                    clearErrors(TilbakekrevingFormField.FRITEKST_VARSEL);
                }
            }}
            onBlur={onBlur}
        ...
        >
            ...
            <Radio value={Tilbakekrevingsvalg.OPPRETT_TILBAKEKREVING_UTEN_VARSEL}>
                Opprett tilbakekreving, ikke send varsel
            </Radio>
            <Radio value={Tilbakekrevingsvalg.IGNORER_TILBAKEKREVING}>Avvent tilbakekreving</Radio>
        </RadioGroup>
    );
}
