...
describe('Dokumentvelger', () => {
    test('viser feilmelding når verken dokumenter eller fritekster er valgt', async () => {
        ...
        expect(
            await screen.findByText('Du må velge minst ett dokument eller legge til en fritekst')
        ).toBeInTheDocument();
    });
    ...
    test('viser ingen feilmelding når minst én fritekst finnes, selv uten valgte dokumenter', async () => {
        ...
        expect(
            screen.queryByText('Du må velge minst ett dokument eller legge til en fritekst')
        ).not.toBeInTheDocument();
    });

    test('valg av et dokument oppdaterer feltet og fjerner feilmeldingen', async () => {
        const { sendInnSkjema, screen, user } = renderMedSkjema(<Dokumentvelger />, {
            defaultValues: { dokumenter: [], fritekster: [] },
        });
        ...
        expect(screen.getByRole('button', { name: `${førsteDokument} slett` })).toBeInTheDocument();
        expect(
            screen.queryByText('Du må velge minst ett dokument eller legge til en fritekst')
        ).not.toBeInTheDocument();
    });
